...
def get_preprocess_fn(name: str) -> Callable[[Dict], Dict]:
    if name == "aime2024":
        ...
        def preprocess_gpqa(x: Dict[str, Any]) -> Dict[str, Any]:
            ...
            return {"question": question, "answer": itos[0], "task": "mc"}
        ...
        def preprocess_mmlu(x: Dict[str, Any]) -> Dict[str, Any]:
            ...
            for i, option in enumerate(options):
                question += f"\n{mmlu_map[i]}: {option}"
            return {"question": question, "temp_answer": mmlu_map[answer], "task": "mc"}
        ...
        def preprocess_mmlu(x: Dict[str, Any]) -> Dict[str, Any]:
            ...
            for i, option in enumerate(options):
                question += f"\n{mmlu_map[i]}: {option}"
            return {"question": question, "answer": answer, "task": "mc"}
        ...
        def preprocess_openbookqa(x: Dict[str, Any]) -> Dict[str, Any]:
            ...
            return {"question": question, "answer": x["answerKey"], "task": "mc"}
    ...
    return dataset.map(
        lambda x: {
            "prompt": format_prompt(
                x[question_key], system_prompt, few_shot, fewshot_prob
            ),
            "answer": x[answer_key],
        },
        num_proc=10,
